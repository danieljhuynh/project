...
int main() {
  ...
  for (int i = 0; i < DefaultListSize; i++) {
    cout << llist.getValue();
    array[DefaultListSize-i-1] = llist.getValue();
    llist.next();
  }
  
  cout << "\nChecking to see if it's a palindrome" << endl;
  llist.movetoStart();
  for (int i =0; i < DefaultListSize/2; i++) {
    cout << "Checking " << llist.getValue() << " with " << array[i] << endl;
    if (llist.getValue() == array[i]) {
